...
export function getAll() {
    return SelectQuery<IBlogTagsRow>('SELECT * FROM blogs JOIN authors ON authors.id = blogs.author.id;');
}

export function getOne(id: number) {
    return SelectQuery<IBlogTagsRow>('SELECT * FROM blogs WHERE id =?;,'[id])
}
